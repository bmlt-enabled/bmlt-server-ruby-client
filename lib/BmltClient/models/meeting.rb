...
module BmltClient
  class Meeting < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BmltClient::Meeting`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'admin_notes')
        self.admin_notes = attributes[:'admin_notes']
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] temporarily_virtual Value to be assigned
    def temporarily_virtual=(temporarily_virtual)
      if temporarily_virtual.nil?
        fail ArgumentError, 'temporarily_virtual cannot be nil'
      end

      @temporarily_virtual = temporarily_virtual
    end
    ...
    def time_zone=(time_zone)
      if time_zone.nil?
        fail ArgumentError, 'time_zone cannot be nil'
      end
