...
module BmltClient
  class RootServer < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BmltClient::RootServer`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] last_successful_import Value to be assigned
    def last_successful_import=(last_successful_import)
      if last_successful_import.nil?
        fail ArgumentError, 'last_successful_import cannot be nil'
      end
